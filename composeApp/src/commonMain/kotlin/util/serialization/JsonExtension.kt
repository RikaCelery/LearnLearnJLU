...
fun JsonElement.ObjectArray(key: String): List<JsonObject> {
    ...
    if (!value.all { it is JsonObject }) throw JsonElementCastException("Values of key '$key' is not all JsonObject, value '$value'")
...
val JsonElement.objectArray: List<JsonObject>
    get() {
        ...
        if (!value.all { it is JsonObject }) throw JsonElementCastException("Values of this array are not all JsonObject, this '$value'")
