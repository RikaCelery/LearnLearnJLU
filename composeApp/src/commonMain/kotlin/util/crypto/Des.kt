/**
 * @author https://github.com/tsurumi-yizhou/JiShi-Android/blob/foss/app/src/main/kotlin/me/yihtseu/jishi/utils/crypto/des.kt
 */
package util.crypto
...
private fun getBoxBinary(i: Int): String {
    ...
    // suing pure kotlin function
    return i.toString(2).padStart(4, '0')
}
...
private fun sBoxPermute(expandByte: ByteArray): ByteArray {
    ...
    val sboxes = listOf(s1, s2, s3, s4, s5, s6, s7, s8)
    for (m in 0 until 8) {
        val i = expandByte[m * 6] * 2 + expandByte[m * 6 + 5]
        val j =
            expandByte[m * 6 + 1] * 2 * 2 * 2 + expandByte[m * 6 + 2] * 2 * 2 + expandByte[m * 6 + 3] * 2 + expandByte[m * 6 + 4]
        getBoxBinary(sboxes[m][i][j]).let {
...
private fun getKeyBytes(key: String): Array<ByteArray?> {
    ...
    val keyBytes = arrayOfNulls<ByteArray>(iterator + (if (key.length % 4 > 0) 1 else 0))
